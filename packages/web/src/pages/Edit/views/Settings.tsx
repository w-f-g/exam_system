...
const EditForm = memo(({ question }: { question: TQuestion }) => {
  const [form] = Form.useForm()
  const { curQuestionId, update } = useContext(QuestionContext)
...
export default function Settings() {
  ...
  const { state, curQuestionId } = useContext(QuestionContext)
  ...
  return (
    <div className="w-[400px] h-full border-l border-l-black">
      ...
      {key === KEY_OPTIONS[1] && question && <EditForm question={question} />}
    </div>
  )
}
