...
import { sendUpdatePasswordCaptcha, updatePassword } from '@/apis'
import { useForm } from 'antd/es/form/Form'
import { IUserUpdatePasswordDto } from '@exam_system/types'
...
export default function UpdatePassword() {
  ...
  const sendCaptcha = async () => {
    ...
    const res = await sendUpdatePasswordCaptcha(address)
    if (res.status === 200 || res.status === 201) {
      message.success('发送成功')
    }
  }

  const onFinish = async (value: IUserUpdatePasswordDto) => {
